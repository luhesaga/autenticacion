...
public interface UserR2DBCDataRepository extends ReactiveCrudRepository<UserEntity, Long> {
    ...
    @Query("SELECT u.*, r.nombre as nombre_rol FROM usuario u JOIN roles r ON u.id_rol = r.id_rol WHERE u.email = :email")
    ...
    @Query("SELECT u.*, r.nombre as nombre_rol FROM usuario u JOIN roles r ON u.id_rol = r.id_rol WHERE u.documento_identidad = :documentoIdentidad")
